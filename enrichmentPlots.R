...
ggplot(topGO, aes(x=prob, y=Mean,colour=group)) + geom_point() +ylab("Mean enrichment") + xlab("Posterior inclusion probability") + ggtitle("Enrichment of terms according to PIP for BMI")
ggsave(filename="Enrichment_bmi.pdf")
...
#topGO$group<-ifelse(df$prob>0.95 & df$Mean>2.0,"Enriched","Non-enriched")
topGO$group<-ifelse(df$prob>0.95 & df$ROPE<0.05,"Enriched","Non-enriched")
ggplot(topGO, aes(x=prob, y=Mean,colour=group)) + geom_point() +ylab("Mean enrichment") + xlab("Posterior inclusion probability") + ggtitle("Enrichment of terms according to PIP for smoking")
...
plot_tissue_enrichment<-function(input,output,title){
 ...
 # tmp<-as.data.frame(enrich.df.PVE$P.VE)
  ...
  p<-ggplot()
  p<-p+ geom_point(data=topGO, aes(x=prob, y=Mean,colour=group)) +ylab("Mean enrichment") + xlab("Posterior inclusion probability") + ggtitle(title)+theme_light()
  p<-p+geom_ribbon(data=ribbon.df,aes(ymin=ymin, ymax=ymax, x=x, fill = "ROPE"), alpha = 0.3)
  ...
  write_delim(x = enrich.df.smry.pxv,delim = " ",path=output[3])#list of go terms for REVIGO
  list(table=enrich.df.smry.pxv, plot.prob=p, plot.prop=q)
}



en_bmi_o<-plot_tissue_enrichment("../SNP+p_bmi_c/enrichmentGTEover.RData",c("probxenrich_GTE_o_bmi.pdf","propxpve_GTE_o_bmi.pdf","GTE_o_bmi.csv"), "Enrichment of GTEX over expressed genes for BMI")

en_bmi_u<-plot_tissue_enrichment("../SNP+p_bmi_c/enrichmentGTEunder.RData",c("probxenrich_GTE_u_bmi.pdf","propxpve_GTE_u_bmi.pdf","GTE_u_bmi.csv"), "Enrichment of GTEX repressed genes for BMI")


en_smk_o<-plot_tissue_enrichment("../SNP+p_smk/enrichmentGTEover.RData",c("probxenrich_GTE_o_smk.pdf","propxpve_GTE_o_smk.pdf","GTE_o_smk.csv"), "Enrichment of GTEX over expressed genes for smoking")

en_smk_u<-plot_tissue_enrichment("../SNP+p_smk/enrichmentGTEunder.RData",c("probxenrich_GTE_u_smk.pdf","propxpve_GTE_u_smk.pdf","GTE_u_bmi.csv"),"Enrichment of GTEX repressed genes for BMI")
...
en_bmi_o_depict<-plot_tissue_enrichment("../SNP+p_bmi_c/enrichmentDEPICTover.RData",c("probxenrich_DEPICT_o_bmi.pdf","propxpve_DEPICT_o_bmi.pdf","DEPICT_o_bmi.csv"), "Enrichment of DEPICT over expressed genes for BMI")
order.table(en_bmi_o_depict$table)

en_bmi_u_depict<-plot_tissue_enrichment("../SNP+p_bmi_c/enrichmentDEPICTunder.RData",c("probxenrich_DEPICT_u_bmi.pdf","propxpve_DEPICT_u_bmi.pdf","DEPICT_u_bmi.csv"), "Enrichment of DEPICT repressed genes for BMI")
order.table(en_bmi_u_depict$table)


en_smk_o_depict<-plot_tissue_enrichment("../SNP+p_smk/enrichmentDEPICTover.RData",c("probxenrich_DEPICT_o_smk.pdf","propxpve_DEPICT_o_smk.pdf","DEPICT_o_smk.csv"), "Enrichment of DEPICT over expressed genes for smoking")
order.table(en_smk_o_depict$table)


en_smk_u_depict<-plot_tissue_enrichment("../SNP+p_smk/enrichmentDEPICTunder.RData",c("probxenrich_DEPICT_u_smk.pdf","propxpve_DEPICT_u_smk.pdf","DEPICT_u_smk.csv"), "Enrichment of DEPICT repressed genes for smoking")
