...
theme_new <- theme_update(axis.text.x = element_text(angle = 45, hjust = 1))
g3<-ggplot()+geom_bar(data=melt(data.frame(Trait=genesEwas$Trait)),mapping=aes(x=Trait)) +ggtitle("Previous probe associations from EWAS catalog")
g3s <- arrangeGrob(g3, top = textGrob("d", x = unit(0, "npc")
         , y   = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Helvetica")))
...
f <- ggplot(g4, aes(x=median, y=reorder(Parameter, median),color=Marker)) + geom_point(size=3) +
        ...
        xlab("HPD") + ylab("Parameter")+theme(axis.text=element_text(size=5))+ggtitle("Effects of markers with 95% IP")+ylab("Marker")+scale_color_discrete(name = "Marker type")+theme_light()
fs <- arrangeGrob(f, top = textGrob("b", x = unit(0, "npc")
         , y   = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Helvetica")))
...
theme_new <- theme_update(axis.text.x = element_text(angle = 45, hjust = 1))
g3<-ggplot()+geom_bar(data=melt(data.frame(Trait=genesEwas$Trait)),mapping=aes(x=Trait)) +ggtitle("Previous probe associations from EWAS catalog")
g3 <- arrangeGrob(g3, top = textGrob("e", x = unit(0, "npc")
         , y   = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Helvetica")))
...
f <- ggplot(g4, aes(x=median, y=reorder(Parameter, median),color=Marker)) + geom_point(size=3) +
        ...
        geom_segment(aes(x=low, xend=high, yend=reorder(Parameter, median),color=Marker), size=0.5) +
        xlab("HPD") + ylab("Parameter")+theme(axis.text=element_text(size=5))+ggtitle("Effects of markers with 95% IP")+ylab("Marker")+scale_color_discrete(name = "Marker type")+theme_light()
f <- arrangeGrob(f, top = textGrob("c", x = unit(0, "npc")
         , y   = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Helvetica")))
#pdf("geneticSummary.pdf")
...
GP=c(result_bmi$genesPerSample,result_smk$genesPerSample)
q<-ggplot()
q<-q+geom_density(data=data.frame(GP=GP,Trait<-Trait),aes(x=GP,fill=Trait),alpha=0.25)+xlab("Genome coverage (methylation probes)")+ggtitle("Per sample genomic coverage")+theme_light()+ scale_fill_manual( values = c("red","blue"))
q <- arrangeGrob(q, top = textGrob("a", x = unit(0, "npc")
         , y   = unit(1, "npc"), just=c("left","top"),
         gp=gpar(col="black", fontsize=18, fontfamily="Helvetica")))
gp<-grid.arrange(q,fs,f,g3s,g3,g2s,g2,nrow=1,layout_matrix=rbind(c(1,1),c(2,3),c(4,5),c(6,7)))
ggsave("gene.pdf",plot=grid.draw(gp),scale=3.2)
