...
corGRTruth<-sqrt(sum(cov(scale(O)%*%B,R)^2))
corGRB<- sqrt(sum((cov(scale(O)%*%meanBetas,R) )^2))
corGRBlup<-sqrt(sum((cov(scale(O)%*%coefBlup,R))^2))
corGRGwas<-sqrt(sum((cov(scale(O)%*%coefGwas,R))^2))
corGRlasso<-sqrt(sum((cov(scale(O)%*%coeflasso,R))^2))
